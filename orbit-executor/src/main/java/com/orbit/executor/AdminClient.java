 ...
 * <p>核心职责：
 * <ul>
 *   <li>负责将执行器的节点信息与心跳定期上报至调度中心集群（支持多地址容灾）；</li>
 *   <li>在执行器服务关闭时，负责向调度中心发送下线请求；</li>
 *   <li>在 HTTP 请求头中携带双向约定的安全访问令牌（{@code X-Orbit-Token}）。</li>
 * </ul>
 */
public class AdminClient {
    ...
    /**
     * 向调度中心发送注册或周期性心跳上报请求。
     * <p>若配置了多个调度中心地址（逗号分隔），将逐个尝试发送，只要有一个成功即判定为本轮心跳成功。
     *
    ...
    /**
     * 向调度中心发送执行器服务下线注销通知。
     * <p>在执行器停机（优雅关闭）时调用，主动摘除注册表中的当前节点，避免调度中心在心跳超时前向已关闭节点分发任务。
     *
