-- ============================================================================
-- Quartz JDBC JobStore 建表脚本（集群模式必需，11 张 QRTZ_* 表）
--
-- 来源：quartz-scheduler/quartz tag v2.5.2（与 orbit-admin/pom.xml 锁定的版本一致）
--       quartz/src/main/resources/org/quartz/impl/jdbcjobstore/tables_gauss_default_compatibility.sql
-- 除以下两处机械改动外，其余内容与官方脚本逐字一致：
--   1) DROP TABLE 语句已注释掉 —— 原脚本是破坏性的，会清空已有调度状态；
--   2) CREATE TABLE / CREATE INDEX 加了 IF NOT EXISTS —— 便于重复执行。
--
-- 适用于 openGauss / GaussDB（默认兼容模式，即 A/PG 兼容）。若实例运行在 M（Oracle）兼容模式，
-- 请改用官方 tables_gauss_m_compatibility.sql，并把 driverDelegateClass 换成
-- org.quartz.impl.jdbcjobstore.StdJDBCDelegate。
--
-- 配套配置（见 orbit-admin/src/main/resources/application-cluster.yml）：
